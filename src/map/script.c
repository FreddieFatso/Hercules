...
int buildin_getareadropitem_sub(struct block_list *bl, va_list ap)
{
	...
	struct flooritem_data *drop = NULL;

	nullpo_ret(bl);
	Assert_ret(bl->type == BL_ITEM);
	drop = BL_UCAST(BL_ITEM, bl);
...
BUILDIN(rid2name) {
	...
	if((bl = map->id2bl(rid))) {
		switch(bl->type) {
			case BL_MOB: script_pushstrcopy(st, BL_UCAST(BL_MOB, bl)->name); break;
			case BL_PC:  script_pushstrcopy(st, BL_UCAST(BL_PC, bl)->status.name); break;
			case BL_NPC: script_pushstrcopy(st, BL_UCAST(BL_NPC, bl)->exname); break;
			case BL_PET: script_pushstrcopy(st, BL_UCAST(BL_PET, bl)->pet.name); break;
			case BL_HOM: script_pushstrcopy(st, BL_UCAST(BL_HOM, bl)->homunculus.name); break;
			case BL_MER: script_pushstrcopy(st, BL_UCAST(BL_MER, bl)->db->name); break;
			default:
