...
int status_check_visibility(struct block_list *src, struct block_list *target) {
	...
	switch ( src->type ) {
	case BL_MOB:
		view_range = BL_UCAST(BL_MOB, src)->min_chase;
		break;
	case BL_PET:
		view_range = BL_UCAST(BL_PET, src)->db->range2;
		break;
	default:
	...
	if ( (tsc = status->get_sc(target)) ) {
		...
		case BL_PC:
			...
			if ((tsc->option&(OPTION_HIDE | OPTION_CLOAK | OPTION_CHASEWALK)
			 ...
			 && !(st->mode&MD_BOSS)
			 && (BL_UCAST(BL_PC, target)->special_state.perfect_hiding || !(st->mode&MD_DETECTOR)))
				return 0;
			break;
		default:
...
unsigned short status_base_matk(struct block_list *bl, const struct status_data *st, int level) {
	...
	switch ( bl->type ) {
		...
		case BL_HOM:
			return status_get_homint(st, BL_UCAST(BL_HOM, bl)) + level;
		case BL_MER:
