...
int battle_gettarget(struct block_list* bl) {
	...
	nullpo_ret(bl);
	switch (bl->type) {
		case BL_PC:  return BL_UCAST(BL_PC, bl)->ud.target;
		case BL_MOB: return BL_UCAST(BL_MOB, bl)->target_id;
		case BL_PET: return BL_UCAST(BL_PET, bl)->target_id;
		case BL_HOM: return BL_UCAST(BL_HOM, bl)->ud.target;
		case BL_MER: return BL_UCAST(BL_MER, bl)->ud.target;
		case BL_ELEM: return BL_UCAST(BL_ELEM, bl)->ud.target;
	}
...
#ifdef RENEWAL
	flag.infdef = skill_id != HT_FREEZINGTRAP ? flag.infdef : 0;
#endif
	if (!flag.infdef && target->type == BL_SKILL) {
		struct skill_unit *su = BL_UCAST(BL_SKILL, target);
		if (su->group->unit_id == UNT_REVERBERATION)
...
#define GET_NORMAL_ATTACK2( f , s ) ( wd.damage2 = battle->calc_base_damage(src, target, s, skill_lv, nk, n_ele, s_ele, s_ele_, EQI_HAND_L, (f), wd.flag) )
	...
	if (wd.damage != 0
	 ...
	 && (src->type != BL_PC
	  || (((struct map_session_data *)src)->weapontype1 == W_DAGGER
	   || ((struct map_session_data *)src)->weapontype1 == W_1HSWORD
...
int battle_check_target( struct block_list *src, struct block_list *target,int flag)
{
	...
	if (s_bl->type == BL_PC) {
		struct map_session_data *s_sd = BL_UCAST(BL_PC, s_bl);
		switch (t_bl->type) {
	...
	switch( target->type ) { // Checks on actual target
		...
		{
			struct status_change *sc = status->get_sc(src);
			struct map_session_data *t_sd = BL_UCAST(BL_PC, target);
	...
	switch( t_bl->type ) { //Checks on target master
		case BL_PC:
		{
			struct map_session_data *sd = BL_UCAST(BL_PC, t_bl);
			if (t_bl == s_bl)
				break;

			if( sd->state.monster_ignore && flag&BCT_ENEMY )
				return 0; // Global immunity only to Attacks
			if (sd->status.karma && s_bl->type == BL_PC && BL_UCAST(BL_PC, s_bl)->status.karma)
		...
		case BL_MOB:
		{
			struct mob_data *md = BL_UCAST(BL_MOB, t_bl);
	...
	switch( src->type ) { //Checks on actual src type
		case BL_PET:
			...
			if (t_bl->type == BL_MOB && BL_UCAST(BL_MOB, t_bl)->guardian_data && flag&BCT_ENEMY)
		...
		case BL_PC:
		{
			...
			if( s_bl != t_bl ) {
				if( sd->state.killer ) {
					...
					if (t_bl->type == BL_PC && sd->duel_group == BL_UCAST(BL_PC, t_bl)->duel_group)
		...
		case BL_MOB:
		{
			...
			if (md->special_state.ai == AI_NONE) {
				...
				if ((target_md != NULL && t_bl->type == BL_PC && target_md->special_state.ai != AI_ZANZOU && target_md->special_state.ai != AI_ATTACK)
				 || (t_bl->type == BL_MOB && BL_UCAST(BL_MOB, t_bl)->special_state.ai == AI_NONE))
					state |= BCT_PARTY; //Normal mobs with no ai are friends.
				else
					state |= BCT_ENEMY; //However, all else are enemies.
			} else {
				if (t_bl->type == BL_MOB && BL_UCAST(BL_MOB, t_bl)->special_state.ai == AI_NONE)
	...
	if( map_flag_vs(m) ) {
		...
		if (state&BCT_ENEMY && battle_config.pk_mode && !map_flag_gvg(m) && s_bl->type == BL_PC && t_bl->type == BL_PC) {
			...
			struct map_session_data *s_sd = BL_UCAST(BL_PC, s_bl);
			struct map_session_data *t_sd = BL_UCAST(BL_PC, t_bl);
			if (
