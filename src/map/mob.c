...
int mob_ai_sub_hard_activesearch(struct block_list *bl,va_list ap)
{
	...
	switch (bl->type) {
		case BL_PC:
			if (BL_UCAST(BL_PC, bl)->state.gangsterparadise && !(status_get_mode(&md->bl)&MD_BOSS))
...
void mob_log_damage(struct mob_data *md, struct block_list *src, int damage)
{
	...
	switch( src->type )
	{
		...
		case BL_MER:
		{
			struct mercenary_data *mer = BL_UCAST(BL_MER, src);
			if( mer->master )
		...
		case BL_MOB:
		{
			struct mob_data *md2 = BL_UCAST(BL_MOB, src);
		...
		case BL_ELEM:
		{
			struct elemental_data *ele = BL_UCAST(BL_ELEM, src);
			if( ele->master )
